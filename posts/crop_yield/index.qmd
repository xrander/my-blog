...
date: "2024-02-10"
categories: [Machine Learning, Multivariate Adaptive Regression Splines (Earth MARS)]
code-fold: true
code-copy: hover
...
pacman::p_load(tidymodels, readr, earth, ggthemr, vip, patchwork)
...
### Parameter Grid
The tuning parameters are updated, the maximum interaction, `prod_degree` used should not be more than 3rd degree, as there's rarely any benefit when it's above such degree. `num_terms` is set to include also possible interaction terms, as data includes `{r} dim(crop_yield)[2]` features.
```{r}
