...
Resamples of the training data was also set at 10 folds, which I think is continuously becoming the go-to value for number of folds `r emo::ji("eyes")`.
...
### Model Specification
As stated earlier `r emo::ji("smug")` decision tree model will be used in this post.
```{r}
...
Decision trees does not require a lot of preprocessing. So, the stating the family will be the only preprocessing step `r emo::ji("eyes)"`.
```{r}
...
The feature perimeter have the largest effect on the model, while solidity have the least effect. Check @fig-cap
